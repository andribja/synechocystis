...
int _fva(glp_prob* P, double* minFlux, double* maxFlux, double* dOptSol,
        double* FBAsol, double* FVAmin, double* FVAmax,
         mwSize  m, mwSize n, double optPercentage,
        int objective, const double* reactions, int nrxn, int verbose);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
   /*
    ...
    Assuming that 64-bit WINGLPK 4.47 has been installed
    >> mex -largeArrayDims -I"C:\matlab\tools\glpkmex\glpk-4.47\src" glpkFVAcc.cpp "C:\matlab\tools\glpkmex\glpk-4.47\w64\glpk_4_47.lib"
   ...
   Warning: No sanity checks are made to see if the function is called with valid arguments.
            Use fastFVA.m to call this function. All bounds are assumed to be fixed
            (+/- Inf not allowed).
   */
   ...
   if (nlhs < 4)
   {
    mexErrMsgTxt("At least four output arguments required.");
   }
   if (nlhs >4 && nlhs != 7)
   {
       mexErrMsgTxt("Exactly seven output arguments are required to get flux values.");
   }
   ...
   plhs[0] = mxCreateDoubleMatrix(n*(n-1)/2,1,mxREAL);
   plhs[1] = mxCreateDoubleMatrix(n*(n-1)/2,1,mxREAL);
   ...
   // Optional arguments
   double *FBAsol = NULL; // Flux vextors corresponding to initial FBA
   double *FVAmin = NULL; // Flux vectors corresponding to FVA minimization
   double *FVAmax = NULL; // Flux vectors corresponding to FVA maximization
   if (nlhs >= 5)
   {
      plhs[4] = mxCreateDoubleMatrix(n,1,mxREAL);
      plhs[5] = mxCreateDoubleMatrix(n,nrxn,mxREAL);
      plhs[6] = mxCreateDoubleMatrix(n,nrxn,mxREAL);
      FBAsol = mxGetPr(plhs[4]);
      FVAmin = mxGetPr(plhs[5]);
      FVAmax = mxGetPr(plhs[6]);
   }

   glp_prob* P;
   P = glp_create_prob();
   _setup_problem(P,c,nz,jc,ir,pr,b,sense,lb,ub,m,n);
   int iRet = _fva(P,minFlux,maxFlux,dOptSol,FBAsol,FVAmin,FVAmax,m,n,optPercentage,objective,rxns,nrxn,verbose);
   *dRetValue=iRet;
...
int _fva(glp_prob* P, double* minFlux, double* maxFlux, double* optSol,
         double* FBAsol, double* FVAmin, double* FVAmax,
         mwSize m, mwSize n, double optPercentage,
         int objective, const double* rxns, int nrxn, int verbose)
{
   ...
   *optSol = z;
   ...
   if (FBAsol != NULL)
   {
      for (int j = 0; j < n; j++)
      {
         FBAsol[j] = glp_get_col_prim(P, j+1);
      }
   }
   ...
   for (int iRound = 0; iRound < 2; iRound++)
   {
      ...
      {
          for(int q = k+1; q < nrxn; q++)
          {
             int j = rxns[k]; // GLPK indices start at 1
             int r = rxns[q];
              mexPrintf("q: %d --- r / rxns[q]: %d --- k: %d --- j / rxns[k]: %d --- count: %d --- iRound: %d\n", q, r, k, j, count, iRound); 
             glp_set_obj_coef(P, j, 1.0);
             glp_set_obj_coef(P, r, -1.0);
             ret = glp_simplex(P, &param); //OPTIMIZE
             glp_set_obj_coef(P, j, 0.0);
             glp_set_obj_coef(P, r, 0.0);
             if (ret != 0)
             {
             ...
             {
                minFlux[count++]= glp_get_obj_val(P);
             }
             else
             {
                maxFlux[count++]=glp_get_obj_val(P);
             }
          }
      }
      count = 0;
